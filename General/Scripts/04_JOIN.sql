...
--> 직접 사용 보단 NATURAL JOIN의 실패 결과로 보여지는 모습이다!
--  라고 알아두는게 좋음
...
FROM EMPLOYEE
NATURAL JOIN JOB;
--> 두 테이블에서 같은 커럼명, 자료형을 지닌 컬럼기준으로 JOIN됨
-- 실패시 결과로 CROSS JOIN 형태가 조회된다
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
...
ORDER BY DEPT_ID DESC, JOB_NAME DESC;
...
JOIN "NATIONAL"s USING (NATIONAL_CODE)
...
-- SELF JOIN 사용
SELECT EMP.EMP_NAME, EMP.DEPT_CODE, MGR.EMP_NAME
FROM EMPLOYEE EMP
LEFT JOIN 
	EMPLOYEE MGR ON (EMP.DEPT_CODE = MGR.DEPT_CODE)
WHERE EMP.EMP_NAME != MGR.EMP_NAME

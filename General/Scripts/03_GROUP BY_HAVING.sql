...
FROM EMPLOYEE
GROUP BY DEPT_CODE; -- 같은 DEPT_CODE끼리 묶음(그룹)
...
SELECT			DEPT_CODE "부서 코드", FLOOR(AVG(SALARY)) "평균 급여"
FROM 				EMPLOYEE
...
ORDER BY 	DEPT_CODE ASC;
-- GROUP BY 로 만들어진 DEPT_CODE 그룹의 HAVING 조건을 만족하는 그룹만 조회
...
MINUS -- 교집합 심봉선, 대북혼 제외한 선행SELECT 결과 조회
